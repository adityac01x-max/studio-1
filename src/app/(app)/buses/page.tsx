...
import { compareBuses, CompareBusesInput, CompareBusesOutput } from '@/ai/flows/compare-buses';
import { Skeleton } from '@/components/ui/skeleton';

type BusResult = CompareBusesOutput['results'][0];
...
export default function BusesPage() {
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        try {
            const input: CompareBusesInput = {
                origin,
                destination,
                journeyDate: format(journeyDate, 'PPP'),
            };
            const response = await compareBuses(input);
                ...
                <CardContent>
                    <form onSubmit={handleSearch} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
                        <div className="space-y-2">
                            ...
                            <Input id="origin" placeholder="e.g., Bangalore" value={origin} onChange={(e) => setOrigin(e.target.value)} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="destination">To</Label>
                            <Input id="destination" placeholder="e.g., Hyderabad" value={destination} onChange={(e) => setDestination(e.target.value)} />
                        </div>
                        <div className="space-y-2">
            ...
            {(isLoading || results.length > 0) && (
                 <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            {isLoading ? `Searching for buses from ${origin} to ${destination}...` : `Showing best prices for buses from ${origin} to ${destination}.`}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <Table>
